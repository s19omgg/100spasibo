...
## Если приветствие приходит несколько раз

Telegram может повторно отправить один и тот же webhook, если ответ от Apps Script пришел медленно или был временный сбой. В `Code.gs` уже есть защита от дублей: обработанные `update_id` сохраняются во временный cache на 6 часов.
